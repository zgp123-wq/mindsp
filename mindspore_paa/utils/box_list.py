...
def ml_nms_mindspore(box, score, label, nms_thresh):
    unique_labels = ops.unique(label).asnumpy().tolist()
    all_keep = []

    # Convert box format to [y1, x1, y2, x2]
    box = ops.stack([box[:, 1], box[:, 0], box[:, 3], box[:, 2]], axis=1)

    for lbl in unique_labels:
        idxs = (label == lbl).nonzero().squeeze()
        sub_boxes = ops.gather(box, idxs)
        sub_scores = ops.gather(score, idxs)
        keep = ops.NMSWithMask(sub_boxes, sub_scores, iou_threshold=nms_thresh)[0]
        all_keep.append(keep + idxs[0])  # adjust index offset

    all_keep = ops.concat(all_keep, axis=0)

    return all_keep

class BoxList:
    def __init__(self, box, img_size, mode, **kwargs):
        assert isinstance(box, mindspore.Tensor), f'box should be a tensor, got {type(box)}'
        ...
        if self.mode == to_mode:
            ...
            if to_mode == 'x1y1x2y2':
                if self.mode == 'x1y1wh':
                    ...
                    self.box = concat_op((x1, y1, x2, y2))
                ...
                if self.mode == 'x1y1x2y2':
                    ...
                    concat_op = P.Concat(axis=1)
                    self.box = concat_op((x1, y1, x2 - x1 + TO_REMOVE, y2 - y1 + TO_REMOVE))
                    self.mode = 'x1y1wh'
                elif self.mode == 'xcycwh':
                    raise NotImplementedError
    ...
    def box_flip(self, method='h_flip'):
        ...
        if self.mode == 'x1y1x2y2':
            ...
            concat_op = P.Concat(axis=1)
            self.box = concat_op((new_x1, new_y1, new_x2, new_y2))
...
def boxlist_ml_nms(box_list, nms_thresh, max_proposals=-1):
    ...
    keep = ml_nms_mindspore(box, score, label.float(), nms_thresh)
...
def cat_boxlist(box_list_fpn):
    ...
    for k in attr:
        if k == 'box':
            ...
            cat_box = concat([getattr(box_list, k) for box_list in box_list_fpn])
